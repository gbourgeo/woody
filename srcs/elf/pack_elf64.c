...
/*   Updated: 2020/11/28 17:25:20 by gbourgeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void pack_elf64(t_env *e)
{
	...
	e->woody_datalen = ((e->banner && *e->banner) ? ft_strlen(e->banner) + 1 : 0) + sizeof(size_t) /* banner length */
...
static void write_new_file(t_env *e, t_elf64 *elf)
{
	...
	if (next && next->p_vaddr - (elf->text_program->p_vaddr + elf->text_program->p_filesz) > woody64_size + e->woody_datalen && 0)

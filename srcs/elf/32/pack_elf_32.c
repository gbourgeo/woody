...
/*   Updated: 2020/12/24 09:53:05 by gbourgeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "main.h"

static void		find_text_first_section(t_elf32 *elf, Elf32_Shdr **first)
{
	*first = NULL;
...
static void		find_text_next_segment(t_elf32 *elf, Elf32_Phdr **next)
{
	*next = NULL;
...
void			pack_elf_32(t_env *e)
{
	...
	if (room_before_next > e->woody_total_size)
		modification_after_text_32(e, &elf);
	/* Or before the first .text segment section */
	else if (room_before_first > e->woody_total_size)
		modification_before_text_32(e, &elf);
	/* Otherwise, enhance .text segment size */
	else
		modification_add_padding_32(e, &elf);
	write_new_file_32(e, &elf);
}
